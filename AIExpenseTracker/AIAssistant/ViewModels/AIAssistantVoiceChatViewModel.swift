...
//  AIExpenseTracker
//
//  Created by P Deepanshu on 16/06/24.
//
    ...
    override func processSpeechTask(audioData: Data) -> Task<Void, Never> {
        Task { @MainActor [unowned self] in
            do {
                ...
                let response = try await functionsManager.prompt(_prompt: prompt, model: model)
                try Task.checkCancellation()
                
                let data = try await api.generateSpeechFrom(input: response.text, voice: .init(rawValue: selectedVoice.rawValue) ?? .alloy)
                try Task.checkCancellation()
