...
//  AIExpenseTracker
//
//  Created by P Deepanshu on 12/06/24.
//
...
class FunctionsManager {
    ...
    var systemText: String {
        "You are expect of tracking and managing expenses logs. Don't make assumption about what values to plug into functions. Ask for clarification if a user request is ambiguous. Current date is \(Self.currentDateFormatter.string(from: .now))"
    }
    ...
    func prompt(_prompt: String, model: ChatGPTModel = .gpt_hyphen_4o, messageID: UUID? = nil) async throws -> AIAssistantResponse {
        do {
            let message = try await api.callFunction(prompt: _prompt, tools: tools, model: model, systemText: systemText)
            ...
            if let toolCall = message.tool_calls?.first,
               ...
               let argumentData = toolCall.function.arguments.data(using: .utf8) {
                ...
                case .addExpenseLog:
                    ...
                    return .init(text: "Please select the confirm button before i add it your expense list", type: .addExpenseLog(.init(log: log, messageID: messageID, userConfirmation: .pending, confirmationCallback: addLogConfirmationCallback)))
                    
                case .listExpenses:
                    guard let listExpenseArgs = try?
                            self.jsonDecoder.decode(ListExpenseArgs.self, from: argumentData)
                    else {
                        throw "Failed to parse function arguments \(toolCall.function.name)\(toolCall.function.arguments)"
                    }
                ...
                case .visualizeExpenses:
                    guard let visualizeExpenseArgs = try? self.jsonDecoder.decode(VisualizeExpenseArgs.self, from: argumentData) else {
                        throw "Failed to parse functions arguments \(toolCall.function.name)\(toolCall.function.arguments)"
                    }
                ...
                default: var text = "Function Name: \(toolCall.function.name)"
                    text += "\nArgs: \(toolCall.function.arguments)"
                    return .init(text: text, type: .contentText)
                }
            } else if let message = message.content {
                api.appendToHistoryList(userText: _prompt, responseText: message)
    ...
    func getQuery(args: ListExpenseArgs) -> Query {
        ...
        if let startDate = args.startDate,
           let endDate = args.endDate {
            filters.append(.whereField("date", isGreaterThan: startDate.startOfDay))
            filters.append(.whereField("date", isLessThanOrEqualTo: endDate.endOfDay))
        } else if let date = args.date {
            filters.append(.whereField("date", isGreaterThan: date.startOfDay))
